...
for (let sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
  ...
  // R8 brand signature
  const brandIssue = await s.evaluate(el => {
    const expected = "「两克伴」出品";
    const forbidden = ["Keke Social Cards", "克克社交卡片"];
    ...
    const hasExpected = text.includes(expected) || cssBrand;
    const bad = forbidden.find(term => text.includes(term));
    if (bad) return { type: "forbidden", bad, expected };
    if (!hasExpected) return { type: "missing", expected };
    return null;
  });
  if (brandIssue) {
    const msg = brandIssue.type === "forbidden"
